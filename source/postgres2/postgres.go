...
func (src *PSQL) setup() (err error) {
	...
	src.sub = pgoutput.NewSubscription(src.conn, src.cfg.replicationSlot, src.cfg.publication, src.cfg.walRetain, true)

	//TODO REMOVE
	sysident, err := pglogrepl.IdentifySystem(context.Background(), src.conn)
	if err != nil {
		return fmt.Errorf("IdentifySystem failed: %v", err)
	}
	src.Errorf(fmt.Sprintf("SystemID: %s  Timeline: %d XLogPos: %d (%s) DBName: %s", sysident.SystemID, sysident.Timeline, sysident.XLogPos, sysident.XLogPos, sysident.DBName))
	//**********
...
func (src *PSQL) Subscribe(sub sink.Sink) {
	src.mux.Subscribe(sub)
}
...
func (src *PSQL) fetch() {
	...
	for {
		...
		start:
			...
			if err = src.sub.Start(src.ctx, walPos, 1, time.Millisecond*100, src.process); err == nil {
				return
			}
...
func (src *PSQL) process(messages []pgoutput.Message, walPos uint64) (err error) {
	for _, m := range messages {
		...
		atomic.StoreUint64(&src.walPosition, walPos)
		...
		src.LogDebugEv(ev.UUID, "Got message '%#v' (wal %d)", m, walPos)
		src.mux.Push(ev, nil)
	}
